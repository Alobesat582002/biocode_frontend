// src/pages/doctor/CreateMedicalLog.jsx

import React, { useState } from 'react';
import { useSearchParams, useNavigate, Link } from 'react-router-dom';
import { FileText, Pill, Plus, Trash2, CheckCircle2, ArrowLeft, Loader2, AlertCircle } from 'lucide-react';
import axiosInstance from '../../api/axiosInstance';

const CreateMedicalLog = () => {
  ...
  if (!patientId) {
    return (
      <div className="flex min-h-[50vh] flex-col items-center justify-center text-center px-4" dir="rtl">
        ...
        <h2 className="text-xl font-bold text-gray-900">مطلوب تحديد المريض</h2>
        <p className="mt-2 max-w-md text-sm text-gray-500">
          لم نتمكن من تحديد المريض المراد إنشاء سجل طبي له. يرجى العودة لصفحة المواعيد واختيار مريض.
        </p>
        <Link 
  ...
  const handleSubmit = async (e) => {
    ...
    if (appointmentId) {
      payload.appointment_id = parseInt(appointmentId, 10);
    }

    try {
      await axiosInstance.post('/api/medical-logs/create/', payload);
      ...
      setError(
        ...
        'فشل حفظ السجل الطبي. يرجى التحقق من البيانات المدخلة.'
      );
    } finally {
      setIsLoading(false);
    }
  ...
  if (isSuccess) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center text-center animate-in fade-in slide-in-from-bottom-4 duration-500" dir="rtl">
        ...
        <h2 className="text-2xl font-bold text-gray-900">تم حفظ السجل الطبي بنجاح</h2>
        <p className="mt-2 max-w-md text-base text-gray-500">
          تم توثيق السجل وجدولة الأدوية للمريض بشكل آمن.
        </p>
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-8 space-y-8" dir="rtl">
      ...
      <div className="flex items-center gap-4 border-b border-gray-100 pb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 sm:text-3xl">إنشاء سجل طبي جديد</h1>
          <p className="mt-1 text-sm text-gray-500">
            اكتب التشخيص ووصف الأدوية بدقة ليتم إضافتها لسجل المريض.
          </p>
        </div>
      </div>
...
export default CreateMedicalLog;
